...
export async function generateMetadata({ params }) {
  const category = await client.fetch(queries.blogCategoryBySlug, { slug: params.slug });
...
export default async function BlogCategoryPage({ params }) {
  try {
    const [category, blogs, allCategories] = await Promise.all([
      client.fetch(queries.blogCategoryBySlug, { slug: params.slug }),
      client.fetch(queries.blogsByCategory, { category: params.slug }),
