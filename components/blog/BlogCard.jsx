...
export default function BlogCard({ blog, featured = false }) {
    ...
    return (
        <Link href={`/blog/${blog.slug || blog.slug?.current}`} className={cardClassName}>
            <article>
                {/* Featured Image */}
                {blog.featuredImage && (
                ...
                <div className={featured ? "p-8" : "p-6"}>
                    <h3 className={`font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors line-clamp-2 ${featured ? 'text-2xl md:text-3xl' : 'text-xl'
                        }`}>
                        {blog.title}
                    </h3>

                    {blog.excerpt && (
                        <p className={`text-gray-600 mb-4 ${featured ? 'text-lg line-clamp-3' : 'text-base line-clamp-2'
                            }`}>
                            {blog.excerpt}
                        </p>
                    )}
