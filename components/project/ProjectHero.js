      ...
      <div className="relative z-10 flex flex-col justify-end h-full p-6 md:p-12">
        <div className="max-w-4xl">
          <div className="flex items-center gap-x-3 mb-3">
            ...
            {propertyTypes && propertyTypes.length > 0 && <span className="bg-black/50 backdrop-blur-sm text-white text-xs font-semibold px-3 py-1 rounded-full">{propertyTypes.join(', ')}</span>}
          </div>

          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-shadow-lg">{title || 'Project'}</h1>
          <p className="text-xl md:text-2xl mt-3 text-gray-200 text-shadow">by <span className="font-semibold">{developerName}</span></p>

          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4 text-gray-200">
            <div className="flex items-center"><MapPin className="w-5 h-5 mr-3 text-blue-300" /><span>{displayAddress}</span></div>
            <div className="flex items-center"><Tag className="w-5 h-5 mr-3 text-blue-300" /><span>{displayPrice}</span></div>
            {handover && <div className="flex items-center"><Calendar className="w-5 h-5 mr-3 text-blue-300" /><span>Handover: <span className="font-semibold">{handover}</span></span></div>}
          </div>
