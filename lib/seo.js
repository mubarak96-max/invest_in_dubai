...
function generatePropertyDescription(property) {
  ...
  // Use plain text description if available, otherwise use the title-based description
  const baseDescription = descriptionPlainText || description;
  const propertyDescription = typeof baseDescription === 'string' ? baseDescription.substring(0, 100) : '';

  return `${title} ${categoryText} in ${locationText}. ${features ? `${features}. ` : ''}${priceText}.${handoverText}.${roiText} ${propertyDescription ? `${propertyDescription}. ` : ''}Premium Dubai real estate with expert guidance. Contact us today!`;
}
...
export function generatePropertyStructuredData(property) {
  ...
  return {
    ...
    "@id": propertyUrl,
    "name": title,
    "description": descriptionPlainText || (typeof description === 'string' ? description : title),
    "url": propertyUrl,
